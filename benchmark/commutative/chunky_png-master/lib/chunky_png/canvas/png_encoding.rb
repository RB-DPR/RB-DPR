...
#        end
        ...
        (0...height).all do |y|
          result[y] = send(encode_method, row(y))
        end
        # Li Lu: We have to separate the computing and merging process
        0.upto(height - 1) do |y|
          stream << result[y]
        end
...
#        end
        ...
        if (filter_method.eql?(:encode_png_str_scanline_paeth) ||
            filter_method.eql?(:encode_png_str_scanline_up))
          streamOrig = stream.dup
          (0...height).all do |y|
            pos = start_pos + y * (line_width + 1)
            prev_pos = (y == 0) ? nil : pos - (line_width + 1)
            send(filter_method, stream, pos, prev_pos, line_width, pixel_size, streamOrig)
          end
        elsif filter_method
          (0...height).all do |y|
            pos = start_pos + y * (line_width + 1)
      ...
      # @param (see #encode_png_str_scanline_none)
      # @return [void]
      def encode_png_str_scanline_up(stream, pos, prev_pos, line_width, pixel_size, origStream = nil)
        origStream = stream if origStream.nil?
        line_width.downto(1) do |i|
          b = prev_pos ? origStream.getbyte(prev_pos + i) : 0
          stream.setbyte(pos + i, (origStream.getbyte(pos + i) - b) & 0xff)
        end
      ...
      # @param (see #encode_png_str_scanline_none)
      # @return [void]
      def encode_png_str_scanline_paeth(stream, pos, prev_pos, line_width, pixel_size, origStream = nil)
        origStream = stream if origStream.nil?
        line_width.downto(1) do |i|
          a = (i > pixel_size) ? origStream.getbyte(pos + i - pixel_size) : 0
          b = (prev_pos) ? origStream.getbyte(prev_pos + i) : 0
          c = (prev_pos && i > pixel_size) ? origStream.getbyte(prev_pos + i - pixel_size) : 0
          p = a + b - c
          pa = (p - a).abs
          pb = (p - b).abs
          pc = (p - c).abs
          pr = (pa <= pb && pa <= pc) ? a : (pb <= pc ? b : c)
          stream.setbyte(pos + i, (origStream.getbyte(pos + i) - pr) & 0xff)
        end
