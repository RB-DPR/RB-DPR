...
class Blackscholes < Benchmark
    ...
    def start()
        ...
        threads = Array.new(@cfg_tnum)
        (0...@cfg_tnum).each{|i|
            
            threads[i] = Thread.new{start_parallel(i)} 
        }
        (0...@cfg_tnum).each{|i|
            threads[i].join
        }
    ...
    def start_parallel(tnum)
    
        start_index = @numOptions / @cfg_tnum * tnum
        end_index = start_index + @numOptions / @cfg_tnum
        end_index = @numOptions if(end_index > @numOptions)
        
        print "thread ", tnum, " from ", start_index, " to ", end_index, "\n"
