 ...
 * governing rights and limitations under the License. Copyright (C) 2001 Alan
 * Moore <alan_moore@gmx.net> Copyright (C) 2001 Chad Fowler
 * <chadfowler@chadfowler.com> Copyright (C) 2001-2002 Benoit Cerrina
 * <b.cerrina@wanadoo.fr> Copyright (C) 2001-2004 Jan Arne Petersen
 * <jpetersen@uni-bonn.de> Copyright (C) 2002-2004 Anders Bengtsson
 * <ndrsbngtssn@yahoo.se> Copyright (C) 2002-2005 Thomas E Enebo <enebo@acm.org>
 * Copyright (C) 2004-2005 Charles O Nutter <headius@headius.com> Copyright (C)
 * 2004 Stefan Matthias Aust <sma@3plus4.de> Copyright (C) 2006 Ola Bini
 * <Ola.Bini@ki.se> Copyright (C) 2006 Daniel Steer <damian.steer@hp.com>
...
import org.jruby.parlib.DotAll;
import org.jruby.parlib.SomeTrue;
...
/**
 * The implementation of the built-in class Array in Ruby. Concurrency: no
 * synchronization is required among readers, but all users must synchronize
 * externally with writers.
 */
...
public class RubyArray extends RubyObject implements List, RandomAccess {
    ...
    private static void concurrentModification(Ruby runtime) {
        throw runtime
                .newConcurrencyError("Detected invalid array contents due to unsynchronized modifications with concurrent users");
    }

    /**
     * DPR added rb_ary_all
     */
    @JRubyMethod(name = "all")
    public IRubyObject dotAll(ThreadContext context, Block block) {
        if (!block.isGiven()) {
            throw context.getRuntime().newLocalJumpErrorNoBlock();
        }
        DotAll.arrayDotAll(block, context, this);
        return this;
    }

    /**
     * DPR added rb_ary_someTrue
     */
    @JRubyMethod(name = "someTrue", required = 1)
    public IRubyObject someTrue(ThreadContext context, IRubyObject arg) {
        if (arg.isNil()) {
            throw context.getRuntime().newErrnoEINVALError("Predicate can't be a nil");
        }
        // System.out.println("Begin sometrue");
        return SomeTrue.arraySomeTrue((RubyProc) arg, context, this);
    }
    ...
    public static final RubyArray newArrayLight(final Ruby runtime) {
        /*
         * Ruby arrays default to holding 16 elements, so we create an ArrayList
         * of the same size if we're not told otherwise
         */
    ...
    private static final ByteList EMPTY_ARRAY_BYTELIST = new ByteList(ByteList.plain("[]"),
            USASCIIEncoding.INSTANCE);
    private static final ByteList RECURSIVE_ARRAY_BYTELIST = new ByteList(ByteList.plain("[...]"),
            USASCIIEncoding.INSTANCE);

    // DPR
    /**
     * Name of the array is necessary for race detector output. So far, the name
     * is set to ARRAY /* Weixing Ji
     */
    ...
    /**
     * offset 0 is reserved for meta-data, all reads and writes shift one
     * location right for example, a[0] is mapped to offset 1, a[1] is mapped to
     * offset 2
     */
    private static final int OBJECT_LEVEL_OFFSET = 0;

    // for one read a[offset]
    private void raceElementRd(int offset, ThreadContext context) {
        if (!RaceDetector.turnOff) {
            RaceDetector.notifyRd(this, getName(), offset);

        }
    }

    // for one write a[offset]
    private void raceElementWrt(int offset, ThreadContext context) {
        if (!RaceDetector.turnOff) {
            RaceDetector.notifyWrt(this, getName(), offset);
        }
    }

    // for multiple reads a[offset...offset+len]
    private void raceElementRd(int offset, int len, ThreadContext context) {

        if (!RaceDetector.turnOff) {
            RaceDetector.notifyRd(this, getName(), offset, len);
        }
    }

    // for multiple writes a[offset...offset+len]
    private void raceElementWrt(int offset, int len, ThreadContext context) {
        if (!RaceDetector.turnOff) {
            RaceDetector.notifyWrt(this, getName(), offset, len);
        }
    }

    // for multiple reads arr[offset...offset+len]
    private static void raceElementRd(RubyArray arr, int offset, int len, ThreadContext context) {
        if (!RaceDetector.turnOff) {
            // TODO:change to a region operation
            for (int i = 0; i < len; i++) {
                RaceDetector.notifyRd(arr, arr.getName(), offset + i);
    ...
    /**
     * Operations may be performed on arrays, including insertion, deletion,
     * reference/get, set
     */

    private static final int ARRAY_INSERT = 0;
    private static final int ARRAY_DEL = 1;
    private static final int ARRAY_REF = 2;
    private static final int ARRAY_SET = 3;

    /**
     * Calculate the offset of array operations This method should be invoked
     ...
     * @param context
     */
    private void raceOffset(int offset, int action, ThreadContext context) {
        if (RaceDetector.turnOff) {
            return;
        }
        int index = 0;
        ...
        if (offset >= 0) {
            ...
            index = offset + realLength + 1;
        }

        // Skip if the index is still negative
        if (index < 0) {
            return;
        }

        if (action == ARRAY_DEL && index < realLength) {
            raceElementWrt(index, realLength - index, context);
        } else if (action == ARRAY_REF) {
            raceElementRd(index, context);
        } else if (action == ARRAY_SET) {
            raceElementWrt(index, context);
        } else if (action == ARRAY_INSERT) {
            if (index >= realLength) {
                raceElementWrt(index, context);
            } else {
                raceElementWrt(index, realLength - index + 1, context);
            }
        }
    }
     ...
     * @param context
     */
    private void raceOffsetLen(int offset, int len, int action, ThreadContext context) {
        if (RaceDetector.turnOff) {
            return;
        }
        ...
        if (action == ARRAY_DEL && index < realLength) {
            ...
            raceElementRd(index, len, context);
        } else if (action == ARRAY_SET) {
            raceElementWrt(index, realLength, context);
        } else if (action == ARRAY_INSERT) {
            if (index >= realLength) {
                raceElementWrt(index, len, context);
            } else {
                raceElementWrt(index, realLength + len - index + 1, context);
            }
        }
    }
    ...
    private RubyArray(Ruby runtime, IRubyObject[] vals) {
        ...
        // DPR
        if (!RaceDetector.turnOff) {
            RaceDetector.initMetaData(values.length);
            RaceDetector.setDepth(this);
        }
        // DPR End
    }
    ...
    /*
     * NEWOBJ and OBJSETUP equivalent fastest one, for shared arrays, optional
     * objectspace
     */
    private RubyArray(Ruby runtime, boolean objectSpace) {
        ...
        // DPR
        if (!RaceDetector.turnOff) {
            // FIXME: values may be null, so I temporarily set metadata to 0 if
            // it's null - Li Lu
            if (values != null) {
                RaceDetector.initMetaData(values.length);
            } else {
                RaceDetector.initMetaData(0);
            }
            RaceDetector.setDepth(this);
        }
        // DPR End
    ...
    private void realloc(int newLength, int valuesLength) {
        ...
        if (newLength > valuesLength) {
            ...
            safeArrayCopy(values, begin, reallocated, 0, valuesLength); // elements
// and trailing nils
        } else {
    ...
    private IRubyObject initializeCommon(ThreadContext context, IRubyObject arg0, IRubyObject arg1,
            Block block) {
        Ruby runtime = context.runtime;
    ...
    public IRubyObject dup() {
        if (metaClass.index != ClassIndex.ARRAY)
            return super.dup();
    ...
    public IRubyObject replace(IRubyObject orig) {
        ...
        // DPR: writes to all localtions as well as the meta-data
        raceElementWrt(0, Math.max(realLength, origArr.realLength) + 1, null);
        // DPR End
    ...
    public IRubyObject to_s() {
        ...
        // DPR:read all locations
        raceElementRd(0, realLength, null);
        // DPR
    ...
    public RubyFixnum hash(ThreadContext context) {
        // DPR:read all locations
        raceElementRd(0, realLength, context);
        // DPR
        Ruby runtime = context.runtime;
    ...
    public RubyFixnum hash19(final ThreadContext context) {
        ...
        return (RubyFixnum) getRuntime().execRecursiveOuter(new Ruby.RecursiveFunction() {
            public IRubyObject call(IRubyObject obj, boolean recur) {
                ...
                long h = realLength;
                if (recur) {
                    h ^= RubyNumeric.num2long(invokedynamic(context, context.runtime.getArray(),
                            HASH));
                } else {
    ...
    /** DPR added for store synchronization **/
    private static volatile boolean storeHasWriter = false;
    /** End **/
    /**
     * rb_ary_store
     */
    public final IRubyObject store(long index, IRubyObject value) {
        ...
        modify();
        /** DPR modified **/
        // Two snapshots on values to check whether another thread came in and
// disturbed my current store
        IRubyObject[] initValues = values;
        IRubyObject[] newValues = null;
        // If the head of the value array changed, the write may not be in the
// new array
        while (storeHasWriter || initValues != newValues) {
            initValues = values;
            // Check whether we need to write the array (or its metadata)
            // realLength is monotonic
            if (index >= realLength) {
                // Update array raw data and realLength in a synchronous fashion
                synchronized (this) {
                    storeHasWriter = true;
                    int valuesLength = 0;
                    // After I grab the built-in lock
                    // check again if we need to resize
                    // (My decision at read time may be stale)
                    if (index >= realLength) {
                        valuesLength = values.length - begin;
                        // Do the write ops
    ...
    public IRubyObject fetch(ThreadContext context, IRubyObject arg0, Block block) {
        ...
        // DPR:read all locations
        // TODO: before or after the if????
        raceElementRd((int) index, context);
        // DPR
    ...
    private static RubyArray aryToAry(IRubyObject obj) {
        ...
        RubyArray arr = new RubyArray(obj.getRuntime(), false); // possibly
// should not in object space
    ...
    private final void splice(long beg, long len, IRubyObject rpl, boolean oneNine) {
        ...
        if (beg >= realLength) {
            len = beg + rlen;
            if (len >= valuesLength)
                spliceRealloc((int) len, valuesLength);
            try {
            ...
            if (len != rlen) {
                safeArrayCopy(values, begin + (int) (beg + len), values, begin + (int) beg + rlen,
    ...
    private final void spliceOne(long beg, IRubyObject rpl) {
        ...
        if (beg >= realLength) {
            ...
            if (alen >= valuesLength)
                spliceRealloc((int) alen, valuesLength);

            if (len == 0) {
                safeArrayCopy(values, begin + (int) beg, values, begin + (int) beg + 1, realLength
                        - (int) beg);
                realLength = alen;
            }
        }
    ...
    private void spliceRealloc(int length, int valuesLength) {
        ...
        if (len > length)
            Helpers.fillNil(vals, length, len, getRuntime());
        begin = 0;
        values = vals;
    }
    ...
    public IRubyObject insert(IRubyObject arg1, IRubyObject arg2) {
        ...
        // DPR
        raceOffset((int) pos, ARRAY_INSERT, null);
        // DPR
        if (pos == -1)
            pos = realLength;
        if (pos < 0)
    ...
    public IRubyObject insert(IRubyObject[] args) {
        ...
        // DPR
        raceOffsetLen((int) pos, args.length, ARRAY_INSERT, null);
        // DPR
    ...
    public final RubyArray aryDup() {
        RubyArray dup = new RubyArray(metaClass.getClassRuntime(), metaClass, values, begin,
                realLength);
        dup.isShared = true;
        isShared = true;
    ...
    public final RubyArray aryDup19() {
        ...
        // dup returns an instance of Array, rather than an instance of the
// subclass
    ...
    private final IRubyObject values_at(long olen, IRubyObject[] args) {
        ...
        for (int i = 0; i < args.length; i++) {
            if (args[i] instanceof RubyFixnum) {
                // DPR
                raceElementRd((int) ((RubyFixnum) args[i]).getLongValue(), null);
                // DPR
    ...
    public IRubyObject subseq(long beg, long len) {
        ...
        if (len == 0)
            return new RubyArray(getRuntime(), getMetaClass(), IRubyObject.NULL_ARRAY);
    ...
    public IRubyObject subseqLight(long beg, long len) {
        ...
        if (len == 0)
            return new RubyArray(runtime, getMetaClass(), IRubyObject.NULL_ARRAY, false);
        return makeShared(begin + (int) beg, (int) len, new RubyArray(runtime, getMetaClass(),
                false));
    }
    ...
    public RubyArray append(IRubyObject item) {
        ...
        // DPR
        raceElementWrt(realLength, null);
        // DPR
    ...
    public RubyArray push_m(IRubyObject[] items) {
        ...
        // DPR
        raceElementWrt(realLength, items.length, null);
        // DPR
        return this;
    }
    ...
    public IRubyObject pop(ThreadContext context) {
        ...
        // DPR
        raceElementWrt(realLength - 1, context);
        // DPR
        if (isShared) {
    ...
    public IRubyObject pop(ThreadContext context, IRubyObject num) {
        ...
        // DPR
        raceElementWrt(realLength - result.realLength, result.realLength, context);
        // DPR
        return result;
    }
    ...
    public IRubyObject shift(ThreadContext context) {
        ...
        // DPR
        raceElementWrt(0, realLength, context);
        // DPR
    ...
    public IRubyObject unshift(IRubyObject item) {
        ...
        values[begin] = item;
        // DPR
        raceElementWrt(0, realLength, null);
        // DPR
        return this;
    }
    ...
    public IRubyObject aref(IRubyObject arg0) {
        ...
        if (arg0 instanceof RubyFixnum) {
            long index = ((RubyFixnum) arg0).getLongValue();
            // DPR
            raceOffset((int) index, ARRAY_REF, null);
            // DPR
    ...
    public IRubyObject aref19(IRubyObject arg0) {
        // return arg0 instanceof RubyFixnum ?
        // entry(((RubyFixnum)arg0).getLongValue()) : arefCommon(arg0);
        if (arg0 instanceof RubyFixnum) {/* Weixing Ji */
            long index = ((RubyFixnum) arg0).getLongValue();
            // DPR
            raceOffset((int) index, ARRAY_REF, null);
            // DPR
    ...
    private IRubyObject arefCommon(IRubyObject arg0) {
        if (arg0 instanceof RubyRange) {
            ...
            // DPR
            raceOffsetLen((int) beglen[0], (int) beglen[1], ARRAY_REF, null);
            // DPR
    ...
    private IRubyObject arefCommon(IRubyObject arg0, IRubyObject arg1) {
        ...
        if (beg < 0)
            beg += realLength;
        // DPR
        raceOffsetLen((int) beg, (int) (RubyNumeric.num2long(arg1)), ARRAY_REF, null);
        // DPR
    ...
    public IRubyObject aset(IRubyObject arg0, IRubyObject arg1) {
        ...
        if (arg0 instanceof RubyFixnum) {
            long index = ((RubyFixnum) arg0).getLongValue();
            // DPR
            raceOffset((int) index, ARRAY_SET, null);
            // DPR
            ...
            // DPR
            raceOffsetLen((int) beg, (int) range.begLen1(realLength, beg), ARRAY_SET, null);
            // DPR
    ...
    public IRubyObject aset(IRubyObject arg0, IRubyObject arg1, IRubyObject arg2) {
        ...
        // DPR
        raceOffsetLen((int) (RubyNumeric.num2long(arg0)), (int) (RubyNumeric.num2long(arg1)),
                ARRAY_SET, null);
        // DPR
    ...
    public RubyArray concat(IRubyObject obj) {
        ...
        if (ary.realLength > 0) {
            // DPR
            raceOffsetLen((int) realLength, ary.realLength + 1, ARRAY_INSERT, null);
            // DPR
            splice(realLength, 0, ary, false);
        }
        return this;
    }
    ...
    private IRubyObject inspectAry(ThreadContext context) {
        ...
        RubyString str = RubyString.newStringLight(context.runtime, DEFAULT_INSPECT_STR_SIZE,
                encoding);
        str.cat((byte) '[');
        ...
        for (int i = 0; i < realLength; i++) {
            ...
            if (i == 0 && str2.getEncoding() != encoding)
                str.setEncoding(str2.getEncoding());
            if (str2.isTaint())
                tainted = true;
            if (str2.isUntrusted())
                untrust = true;
    ...
    public IRubyObject inspect() {
        if (realLength == 0)
            return RubyString.newStringShared(getRuntime(), EMPTY_ARRAY_BYTELIST);
        if (getRuntime().isInspecting(this))
            return RubyString.newStringShared(getRuntime(), RECURSIVE_ARRAY_BYTELIST);
    ...
    public IRubyObject last() {
        if (realLength == 0)
            return getRuntime().getNil();
        // DPR
        raceElementRd(realLength - 1, null);
        // DPR
    ...
    public IRubyObject last(IRubyObject arg0) {
        ...
        // DPR
        raceElementRd(realLength - (int) n - 1, (int) n, null);
        // DPR
    ...
    public IRubyObject eachCommon(ThreadContext context, Block block) {
        ...
        for (int i = 0; i < realLength; i++) {
            // do not coarsen the "safe" catch, since it will misinterpret
// AIOOBE from the yielded code.
            // See JRUBY-5434
    ...
    public IRubyObject each(ThreadContext context, Block block) {
        return block.isGiven() ? eachCommon(context, block) : enumeratorize(context.runtime, this,
                "each");
    }
    ...
    public IRubyObject eachSlice(ThreadContext context, int size, Block block) {
        ...
        for (; localRealLength >= size; localRealLength -= size) {
            ...
            if (specificArity) { // array is never exposed to ruby, just use for
// yielding
    ...
    public IRubyObject each_slice(ThreadContext context, IRubyObject arg, Block block) {
        ...
        return block.isGiven() ? eachSlice(context, size, block) : enumeratorize(context.runtime,
                this, "each_slice", arg);
    }
    ...
    public IRubyObject each_index(ThreadContext context, Block block) {
        return block.isGiven() ? eachIndex(context, block) : enumeratorize(context.runtime, this,
                "each_index");
    }
...
// AIOOBE from the yielded code.
    ...
    public IRubyObject reverse_each(ThreadContext context, Block block) {
        return block.isGiven() ? reverseEach(context, block) : enumeratorize(context.runtime, this,
                "reverse_each");
    }
...
// AIOOBE from inspect.
    ...
    // 1.9 MRI: ary_join_1
    private RubyString joinAny(ThreadContext context, IRubyObject obj, RubyString sep, int i,
            RubyString result) {
    ...
    public IRubyObject join19(final ThreadContext context, IRubyObject sep) {
        ...
        for (int i = begin; i < begin + realLength; i++) {
            IRubyObject val = safeArrayRef(values, i);
            // DPR
            raceElementRd(i - begin, context);
            // DPR
            IRubyObject tmp = val.checkStringType19();
            if (tmp.isNil() || tmp != val) {
                len += ((begin + realLength) - i) * 10;
                final RubyString result = (RubyString) RubyString.newStringLight(runtime, len,
                        USASCIIEncoding.INSTANCE).infectBy(this);
                ...
                return runtime.recursiveListOperation(new Callable<IRubyObject>() {
                    public IRubyObject call() {
                        return joinAny(context, RubyArray.this, sepStringFinal, iFinal,
                                joinStrings(sepStringFinal, iFinal, result));
                    }
                });
            }
    ...
    public RubyBoolean compare(ThreadContext context, MethodNames method, IRubyObject other) {
        ...
        for (int i = 0; i < realLength; i++) {
            ...
            if (a == b)
                continue; // matching MRI opt. mock frameworks can throw errors
// if we don't

            if (!invokedynamic(context, a, method, b).isTrue())
                return context.runtime.getFalse();
        }
    ...
    public IRubyObject compact_bang() {
        ...
        // DPR
        raceElementWrt(start - begin, realLength - start + 1, null);
        // DPR
    ...
    @JRubyMethod
    public IRubyObject fill(ThreadContext context, IRubyObject arg1, IRubyObject arg2,
            IRubyObject arg3, Block block) {
        if (block.isGiven()) {
    ...
    private IRubyObject fillCommon(ThreadContext context, int beg, long len, Block block) {
        ...
        // DPR
        raceElementWrt(beg, (int) len, context);
        // DPR End
        return this;
    }
    ...
    public IRubyObject bsearch(ThreadContext context, Block block) {
        ...
        while (low < high) {
            ...
            if (v instanceof RubyFixnum) {
                ...
                switch (RubyComparable.cmpint(context,
                        invokedynamic(context, v, OP_CMP, RubyFixnum.zero(runtime)), v,
                ...
                throw runtime.newTypeError("wrong argument type " + v.getType().getName()
    ...
    public IRubyObject indexes(IRubyObject[] args) {
        getRuntime().getWarnings().warn(ID.DEPRECATED_METHOD,
                "Array#indexes is deprecated; use Array#values_at");
...
// AIOOBE from the yield
    ...
    public IRubyObject collect19(ThreadContext context, Block block) {
        return block.isGiven() ? collect(context, block) : enumeratorize(context.runtime, this,
                "collect");
    }
...
// AIOOBE from the yield
    ...
    public IRubyObject collect_bang(ThreadContext context, Block block) {
        return block.isGiven() ? collectBang(context, block) : enumeratorize(context.runtime, this,
                "collect!");
    }
...
// AIOOBE from the yield
    ...
    public IRubyObject select(ThreadContext context, Block block) {
        return block.isGiven() ? selectCommon(context, block) : enumeratorize(context.runtime,
                this, "select");
    }
    ...
    public IRubyObject select_bang(ThreadContext context, Block block) {
        ...
        if (realLength == newLength)
            return runtime.getNil(); // No change
        // DPR
        raceElementWrt(modifyIndex, realLength - modifyIndex + 1, context);
        // DPR
...
// AIOOBE from equalInternal
        ...
        // DPR
        raceElementWrt(start, realLength - start + 1, context);
        // DPR
        modify();
    ...
    public final IRubyObject delete_at(int pos) {
        ...
        if (pos >= len || (pos < 0 && (pos += len) < 0))
            return getRuntime().getNil();
        // DPR
        raceOffsetLen((int) pos, (int) (len - pos), ARRAY_DEL, null);
        // DPR
    ...
    public IRubyObject reject(ThreadContext context, Block block) {
        ...
        raceElementRd(0, realLength, context);
        // DPR
        return block.isGiven() ? rejectCommon(context, block) : enumeratorize(context.runtime,
                this, "reject");
    }
...
// AIOOBE from the yield
    ...
    public IRubyObject reject_bang(ThreadContext context, Block block) {
        ...
        // enumeratorize(context.getRuntime(), this, "reject!");
        // DPR
        if (block.isGiven()) {
            return rejectBang(context, block);
        } else {
            raceElementRd(0, realLength, context);
            return enumeratorize(context.getRuntime(), this, "reject!");
        }
        // DPR
    }
    ...
    public IRubyObject delete_if(ThreadContext context, Block block) {
        if (block.isGiven()) {
            return deleteIf(context, block);
        } else {
            // DPR
            raceElementRd(0, realLength, context);
            // DPR
            return enumeratorize(context.getRuntime(), this, "delete_if");
        }

        // return block.isGiven() ? deleteIf(context, block) :
// enumeratorize(context.runtime, this, "delete_if");
    }
    ...
    // This can be shared with RubyEnumerable to clean #zipCommon{Enum,Arg} a
// little
    public static interface ArgumentVisitor {
        IRubyObject visit(ThreadContext ctx, IRubyObject arg, int i);
    }

    private IRubyObject zipCommon(ThreadContext context, IRubyObject[] args, Block block,
...
// AIOOBE from the yield
    ...
    public IRubyObject op_cmp(ThreadContext context, IRubyObject obj) {
        ...
        boolean isAnArray = (obj instanceof RubyArray)
                || obj.getMetaClass().getSuperClass() == runtime.getArray();
    ...
    public IRubyObject slice_bang(IRubyObject arg0) {
        ...
        if (arg0 instanceof RubyRange) {
            ...
            // DPR
            raceElementWrt((int) pos, realLength - ((int) pos) + 1, null);
            // DPR
    ...
    public IRubyObject op_times(ThreadContext context, IRubyObject times) {
        ...
        Ruby runtime = context.runtime;
        if (len == 0)
            return new RubyArray(runtime, getMetaClass(), IRubyObject.NULL_ARRAY);
        if (len < 0)
    ...
    public IRubyObject op_times19(ThreadContext context, IRubyObject times) {
        ...
        Ruby runtime = context.runtime;
        if (len == 0)
            return new RubyArray(runtime, getMetaClass(), IRubyObject.NULL_ARRAY).infectBy(this);
        if (len < 0)
            throw runtime.newArgumentError("negative argument");
    ...
    public IRubyObject uniq_bang(ThreadContext context) {
        ...
        // DPR
        if (index == -1)
            raceElementRd(index, realLength - index + 1, context);
        else {
    ...
    public IRubyObject op_and(IRubyObject other) {
        ...
        RubyArray ary3 = new RubyArray(getRuntime(), realLength < ary2.realLength ? realLength
                : ary2.realLength);
    ...
    public IRubyObject sort_bang(ThreadContext context, Block block) {
        ...
        if (realLength > 1) {
            ...
            try {
                return block.isGiven() ? sortInternal(context, block)
                        : sortInternal(context, false);
            } finally {
                flags &= ~TMPLOCK_ARR_F;
            }
        }
    ...
    private IRubyObject sortInternal(final ThreadContext context, boolean honorOverride) {
        ...
        final boolean fixnumBypass = !honorOverride
                || context.runtime.newFixnum(0).isBuiltin("<=>");
        final boolean stringBypass = !honorOverride
                || context.runtime.newString("").isBuiltin("<=>");
    ...
    private IRubyObject sortInternal(final ThreadContext context, final Block block) {
        ...
        Qsort.sort(newValues, 0, length, new Comparator() {
            public int compare(Object o1, Object o2) {
                ...
                IRubyObject ret = block.yieldArray(context, getRuntime().newArray(obj1, obj2),
                        null, null);
                // TODO: ary_sort_check should be done here
    ...
    public IRubyObject sort_by_bang(ThreadContext context, Block block) {
        if (!block.isGiven())
            return enumeratorize(context.runtime, this, "sort_by!");
    ...
    public IRubyObject drop(ThreadContext context, IRubyObject n) {
        ...
        // DPR
        raceElementRd(0, (int) pos, context);
        // DPR
    ...
    public IRubyObject cycle(ThreadContext context, Block block) {
        ...
        // DPR
        if (!block.isGiven())
            return enumeratorize(context.runtime, this, "cycle");
    ...
    public IRubyObject cycle(ThreadContext context, IRubyObject arg, Block block) {
        ...
        // DPR
        if (!block.isGiven())
            return enumeratorize(context.runtime, this, "cycle", arg);
    ...
    public IRubyObject product19(ThreadContext context, IRubyObject[] args, Block block) {
        ...
        for (int i = 0; i < resultLen; i++) {
            ...
            for (int j = 0; j < n; j++)
                sub.append(arrays[j].entry(counters[j]));
    ...
    public IRubyObject combination(ThreadContext context, IRubyObject num, Block block) {
        ...
        Ruby runtime = context.runtime;
        if (!block.isGiven())
            return enumeratorize(runtime, this, "combination", num);
    ...
    public IRubyObject repeatedCombination(ThreadContext context, IRubyObject num, Block block) {
        ...
        Ruby runtime = context.runtime;
        if (!block.isGiven())
            return enumeratorize(runtime, this, "repeated_combination", num);
    ...
    private static void repeatCombination(ThreadContext context, Ruby runtime,
            IRubyObject[] values, int[] stack, int index, int max, Block block) {
    ...
    private void permute(ThreadContext context, int n, int r, int[] p, int index, boolean[] used,
            boolean repeat, RubyArray values, Block block) {
        for (int i = 0; i < n; i++) {
            if (repeat || !used[i]) {
                p[index] = i;
                if (index < r - 1) {
    ...
    public IRubyObject permutation(ThreadContext context, IRubyObject num, Block block) {
        ...
        return block.isGiven() ? permutationCommon(context, RubyNumeric.num2int(num), false, block)
                : enumeratorize(context.runtime, this, "permutation", num);
    ...
    public IRubyObject permutation(ThreadContext context, Block block) {
        ...
        return block.isGiven() ? permutationCommon(context, realLength, false, block)
                : enumeratorize(context.runtime, this, "permutation");
    }
    ...
    public IRubyObject repeated_permutation(ThreadContext context, IRubyObject num, Block block) {
        ...
        return block.isGiven() ? permutationCommon(context, RubyNumeric.num2int(num), true, block)
                : enumeratorize(context.runtime, this, "repeated_permutation", num);
    }
    ...
    private IRubyObject permutationCommon(ThreadContext context, int r, boolean repeat, Block block) {
        if (r == 0) {
            ...
            int n = realLength;
            permute(context, n, r, new int[r], 0, new boolean[n], repeat,
    ...
    public IRubyObject shuffle_bang(ThreadContext context, IRubyObject[] args) {
        ...
        if (args.length > 0) {
            ...
            if (!hash.isNil()) {
                IRubyObject argRandgen = ((RubyHash) hash).fastARef(context.runtime
                        .newSymbol("random"));
                if (argRandgen != null) {
                    randgen = argRandgen;
                }
    ...
    public IRubyObject sample(ThreadContext context, IRubyObject[] args) {
        ...
        try {
            ...
            if (args.length == 0) {
                ...
                raceElementRd(begin, realLength - begin + 1, context); /*
                                                                        * Weixing
                                                                        * Ji
                                                                        */
                return eltOk(i);
            ...
            case 3:
                ...
                if (k >= l && (++k >= g))
                    ++k;
                return new RubyArray(runtime, new IRubyObject[] { eltOk(i), eltOk(j), eltOk(k) });
            }
    ...
    public IRubyObject all_p(ThreadContext context, Block block) {
        if (!isBuiltin("each"))
            return RubyEnumerable.all_pCommon(context, this, block, Arity.OPTIONAL);
        if (!block.isGiven())
            return all_pBlockless(context);
    ...
    public IRubyObject any_p(ThreadContext context, Block block) {
        if (!isBuiltin("each"))
            return RubyEnumerable.any_pCommon(context, this, block, Arity.OPTIONAL);
        if (!block.isGiven())
            return any_pBlockless(context);
    ...
    public IRubyObject find_index(ThreadContext context, Block block) {
        if (!isBuiltin("each"))
            return RubyEnumerable.find_indexCommon(context, this, block, Arity.OPTIONAL);
    ...
    public IRubyObject detectCommon(ThreadContext context, IRubyObject ifnone, Block block) {
        ...
        return ifnone != null ? ifnone.callMethod(context, "call") : context.runtime.getNil();
    }
    ...
    public static RubyArray unmarshalFrom(UnmarshalStream input) throws IOException {
        ...
        // we create this now with an empty, nulled array so it's available for
// links in the marshal data
     ...
     * specified offset. It is expected that the target array is large enough to
    ...
    public Object[] toArray() {
        // no catch for ArrayIndexOutOfBounds here because this impls a List
// method
...
// method
    ...
    public Object toJava(Class target) {
        if (target.isArray()) {
            ...
            try {
                ArrayUtils.copyDataToJavaArrayDirect(getRuntime().getCurrentContext(), this,
                        rawJavaArray);
            } catch (ArrayIndexOutOfBoundsException aioob) {
                concurrentModification();
            }
    ...
    public boolean removeAll(Collection c) {
        ...
        for (Iterator iter = c.iterator(); iter.hasNext();) {
            IRubyObject deleted = delete(getRuntime().getCurrentContext(),
                    JavaUtil.convertJavaToUsableRubyObject(getRuntime(), iter.next()),
                    Block.NULL_BLOCK);
            if (!deleted.isNil()) {
                listChanged = true;
            }
    ...
    public void add(int index, Object element) {
        insert(new IRubyObject[] { RubyFixnum.newFixnum(getRuntime(), index),
                JavaUtil.convertJavaToUsableRubyObject(getRuntime(), element) });
    }
    ...
    public int indexOf(Object element) {
        ...
        if (element != null) {
            IRubyObject convertedElement = JavaUtil.convertJavaToUsableRubyObject(getRuntime(),
                    element);
    ...
    final class RubyArrayConversionListIterator extends RubyArrayConversionIterator implements
            ListIterator {
        ...
        public void add(Object obj) {
            insert(new IRubyObject[] { RubyFixnum.newFixnum(getRuntime(), index++),
                    JavaUtil.convertJavaToUsableRubyObject(getRuntime(), obj) });
            last = -1;
        }
    }
    ...
    // TODO: list.subList(from, to).clear() is supposed to clear the sublist
// from the list.
    ...
    private static IRubyObject safeArraySet(Ruby runtime, IRubyObject[] values, int i,
            IRubyObject value) {
        try {
    ...
    private static IRubyObject safeArrayRefSet(Ruby runtime, IRubyObject[] values, int i,
            IRubyObject value) {
        try {
    ...
    private IRubyObject safeArrayRefCondSet(IRubyObject[] values, int i, boolean doSet,
            IRubyObject value) {
        return safeArrayRefCondSet(getRuntime(), values, i, doSet, value);
    }

    private static IRubyObject safeArrayRefCondSet(Ruby runtime, IRubyObject[] values, int i,
            boolean doSet, IRubyObject value) {
        try {
            IRubyObject tmp = values[i];
            if (doSet)
                values[i] = value;
    ...
    private void safeArrayCopy(IRubyObject[] source, int sourceStart, IRubyObject[] target,
            int targetStart, int length) {
        safeArrayCopy(getRuntime(), source, sourceStart, target, targetStart, length);
    }

    private void safeArrayCopy(Ruby runtime, IRubyObject[] source, int sourceStart,
            IRubyObject[] target, int targetStart, int length) {
        try {
