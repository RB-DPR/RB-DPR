...
import org.jruby.race.RaceMetaData;
...
public class RubyBasicObject implements Cloneable, IRubyObject, Serializable, Comparable<IRubyObject>, CoreObjectType, InstanceVariables, InternalVariables {
    ...
    // DPR
    /*
     * Shadow memory for Nondeterminator and SPD3
	 *  
	 */
	protected RaceMetaData raceMetaData = null;
	
	/*
	 * Get shadow memory for Nondeterminator
	 */
	public ProcNode[] getCilkShadow() {
	    if (raceMetaData == null) {
	        // If I'm null, the caller will set up my shadow
	        return null;
	    }
	    return (ProcNode[]) (raceMetaData.getMetaData());
	}

	/*
     * Get shadow memory for SPD3
     */
	public Object[] getSPD3Shadow() {
	    if (raceMetaData == null) {
	        // If I'm null, the caller will set up my shadow
	        return null;
	    }
	    return raceMetaData.getMetaData();
	}

	/*
     * Set shadow memory for Nondeterminator
     */
	public void setCilkShadow(ProcNode[] shadow) {
	    if (raceMetaData == null)
	        raceMetaData = RaceMetaData.createMetaData();
	    raceMetaData.setMetaData(shadow);
	}

	/*
     * Get shadow memory for SPD3
     */
	public void setSPD3Shadow(Object[] shadow) {
	    if (raceMetaData == null){
		synchronized(this){
		    if(raceMetaData == null){
	        	raceMetaData = RaceMetaData.createMetaData();
	...
	/*
     * Get the sequential number of SeqLock
     */
	public long getSeqNum() {
	    ...
	    return raceMetaData.getSeqNum();
	}

	/*
     * Change the sequential number of SeqLock
     */
	public boolean casSeqNum(long oldv, long newv){
	    if (raceMetaData == null){
	        synchronized(this){
	            if(raceMetaData == null){
	                raceMetaData = RaceMetaData.createMetaData();
	    ...
	    return raceMetaData.casSeqNum(oldv, newv);
	}

	/*
     * Increase the sequential number of SeqLock
     */
	public void insSeqNum() {
	    if (raceMetaData == null){
	        synchronized(this){
	            if(raceMetaData == null){
	                raceMetaData = RaceMetaData.createMetaData();
    ...
    //DPR
    /**
     *  This flag is used to remember the depth of the task that creates this
     *  object in the task tree. 
     */
	public static final int FL_DSHIFT = 16;
   ...
   protected final void testFrozen() {
    ...
    /**
     * Set the depth of task that creates the object, 8 bits are used
     */
    public final void setDepth(int depth) {
        // clear the previous value
        flags = flags & 0xffff;
        // set the new depth
        flags = flags + ((depth & 0xff) << this.FL_DSHIFT);
    }

    /**
     * Get the depth of task that creates the object
     */
    public final int getDepth() {
        return ((flags >> this.FL_DSHIFT) & 0xff);
    }
