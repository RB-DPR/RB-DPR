...
public class RubyProc extends RubyObject implements DataType {
    ...
    // Added for DPR
    @JRubyMethod(name = ">>", required = 1) 
    public IRubyObject pipeCombine(ThreadContext context, IRubyObject newStage) {
        RubyPipeline pipe = (RubyPipeline)RubyPipeline.newInstance(context);
        pipe.push(context, this);
        pipe.push(context, newStage);
        return pipe;
    }
