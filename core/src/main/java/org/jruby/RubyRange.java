...
public class RubyRange extends RubyObject {
    ...
    public IRubyObject all(final ThreadContext context, final Block block) {
        Ruby runtime = context.getRuntime();
        if (!block.isGiven()) return enumeratorize(runtime, this, "each");

        if (begin instanceof RubyFixnum && end instanceof RubyFixnum) {
            DotAll.fixnumRangeDotAll(block, context, (RubyFixnum) begin, (RubyFixnum) end, isExclusive);
        } else {
            if (!begin.respondsTo("succ")) throw getRuntime().newTypeError(
                    "can't call .all from " + begin.getMetaClass().getName());
