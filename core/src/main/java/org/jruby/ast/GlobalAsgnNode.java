    ...
    public IRubyObject interpret(Ruby runtime, ThreadContext context, IRubyObject self, Block aBlock) {
    	...
    	if(!RaceDetector.turnOff)
    	{
    	    /*if(RaceDetector.raceType == RaceDetector.RACE_TYPE_LOG)
	    		RaceDetector.notifyWrt(((RubyBasicObject)self).getObjectId(),
				                            this.name,
				                            0);
	    	else if(RaceDetector.raceType == RaceDetector.RACE_TYPE_SPD3){
				RaceDetector.rdWrtCheck((RubyBasicObject) self, 0, 1);
			} else if (RaceDetector.raceType == RaceDetector.RACE_TYPE_CILK) {
				RaceDetector.writeCilk(((RubyBasicObject) self).getObjectId(),
						((RubyBasicObject) self).getCilkShadow(), 0);
			}*/
    		RaceDetector.notifyWrt(self, this.name, 0);

			/*RubyThread  rt = context.getThread();
			rt.re.eventId = RaceDetector.RACE_EVENT_WRITE;
			rt.re.objId = ((RubyBasicObject)self).getObjectId();
			rt.re.offset = 0;
			rt.re.info = this.name;
			runtime.rd.notify(rt.re);*/
		}
		/*Weixing Ji*/
