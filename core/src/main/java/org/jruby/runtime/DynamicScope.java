...
public abstract class DynamicScope {
    ...
    // DPR
    // the unique id for a DynamicScope
    private long id = -1;   
    // depth of the scope
	protected int depth;
	// metadata for shadow based detector
	protected Object[] varMetaData = null;
	// Seqlock
    protected AtomicLong seqNumMd = new AtomicLong(0);
	
    // Li Lu: Shadow memory for cilk
    protected ProcNode[] cilkShadow = null;
	...
	/**
	 * set the depth of current scope
	 * @param d
	 */
    ...
    // DPR
    /**
     * get the object id for current scope
     * @return
     */
	public long getObjectId()
	{
		if(id == -1){
			id = RaceDetector.getNextScopeId();
		}
		return id;
	}
    ...
    public boolean casSeqNum(long oldv, long newv){
    	return seqNumMd.compareAndSet(oldv, newv);
    }
    ...
    public ProcNode[] getCilkShadow() {
        return this.cilkShadow;
    }
    
    public void setCilkShadow(ProcNode[] shadow) {
        this.cilkShadow = shadow;
    }

    /**
     * Notify the race detector that current scope is read or write
     * @param offset
     * @param type
     * @param length
     */
	public void notifyRaceDetector(int offset, int type, int length){
		this.length = length;
		if(!RaceDetector.turnOff){
			if(type == 0){
				RaceDetector.notifyRdCheck(this, staticScope, offset, this.depth);
			}
			else{
				RaceDetector.notifyWrtCheck(this, staticScope, offset, this.depth);
			}
 		}
	}
    ...
    public static DynamicScope newDynamicScope(StaticScope staticScope, int depth) {
        //DPR
        DynamicScope s = newDynamicScope(staticScope, null);
        s.setDepth(depth + 1);
        return s;
        //END DPR
