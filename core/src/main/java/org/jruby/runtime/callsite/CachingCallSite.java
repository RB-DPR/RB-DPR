...
//    private AtomicBoolean isPolymorphic = new AtomicBoolean(false);
    ...
    public IRubyObject call(ThreadContext context, IRubyObject caller, IRubyObject self, IRubyObject... args) {
        ...
        if (CacheEntry.typeOk(myCache, selfType)) {
            // for commutative check
            IRubyObject rtn = null;
            if (!RaceDetector.turnOff && !myCache.method.shouldInstrument()) {
                RaceDetector.setInstrumentOff();
            } else if (!RaceDetector.turnOff
                    && myCache.method.isCommutativeAnnotated()) {
                // If the method is commutative annotated, mark it's in
                RaceDetector.setCommutativeIn(self);
            }
            rtn = myCache.method
                    .call(context, self, selfType, methodName, args);
            if (!RaceDetector.turnOff && !myCache.method.shouldInstrument()) {
                RaceDetector.setInstrumentOn();
            } else if (!RaceDetector.turnOff
                    && myCache.method.isCommutativeAnnotated()) {
                RaceDetector.setCommutativeOut();
            }
            // for commutative check
            instrumentCall(self, myCache.method, args, rtn);
            return rtn;
        }
    ...
    private IRubyObject callBlock(ThreadContext context, IRubyObject caller, IRubyObject self, IRubyObject[] args, Block block) {
        ...
        if (CacheEntry.typeOk(myCache, selfType)) {
            ...
            rtn = myCache.method.call(context, self, selfType, methodName,
                    args, block);
            if (!RaceDetector.turnOff && !myCache.method.shouldInstrument()) {
                RaceDetector.setInstrumentOn();
            } else if (!RaceDetector.turnOff
    ...
    public IRubyObject call(ThreadContext context, IRubyObject caller, IRubyObject self) {
        ...
        if (CacheEntry.typeOk(myCache, selfType)) {
            // for commutative check
            IRubyObject rtn = null;
            // Notice on new pattern: shouldInstrument is now connected with
            // unchecked, but not commutativity
            if (!RaceDetector.turnOff && !myCache.method.shouldInstrument()) {
                RaceDetector.setInstrumentOff();
            } else if (!RaceDetector.turnOff
                    && myCache.method.isCommutativeAnnotated()) {
            ...
            // for commutative check
            instrumentCall(self, myCache.method, (IRubyObject) null, rtn);
            return rtn;
        }
    ...
    public IRubyObject call(ThreadContext context, IRubyObject caller, IRubyObject self, IRubyObject arg1) {
        ...
        if (CacheEntry.typeOk(myCache, selfType)) {
            ...
            rtn = myCache.method
                    .call(context, self, selfType, methodName, arg1);
            if (!RaceDetector.turnOff && !myCache.method.shouldInstrument()) {
                RaceDetector.setInstrumentOn();
            } else if (!RaceDetector.turnOff
                    && myCache.method.isCommutativeAnnotated()) {
            ...
            instrumentCall(self, myCache.method, arg1, rtn);
            return rtn;
        }
    ...
    public IRubyObject call(ThreadContext context, IRubyObject caller, IRubyObject self, IRubyObject arg1, IRubyObject arg2, IRubyObject arg3) {
        ...
        if (CacheEntry.typeOk(myCache, selfType)) {
            ...
            IRubyObject[] args = { arg1, arg2, arg3 };
            instrumentCall(self, myCache.method, args, rtn);
            return rtn;
        }
    ...
    protected IRubyObject cacheAndCall(IRubyObject caller, RubyClass selfType, Block block, IRubyObject[] args, ThreadContext context, IRubyObject self) {
        ...
        updateCache(entry);
        IRubyObject rtn = null;
        if(!RaceDetector.turnOff && !method.shouldInstrument()){
    		RaceDetector.setInstrumentOff();
    	} else if (!RaceDetector.turnOff && method.isCommutativeAnnotated()) {
            // If the method is commutative annotated, mark it's in
            RaceDetector.setCommutativeIn(self);
        }
        rtn = method.call(context, self, selfType, methodName, args, block);
        if(!RaceDetector.turnOff && !method.shouldInstrument()){
    		RaceDetector.setInstrumentOn();
    	} else if (!RaceDetector.turnOff && method.isCommutativeAnnotated()) {
            RaceDetector.setCommutativeOut();
        ...
        //for commutative check
        instrumentCall(self, method, args, rtn);
        return rtn;
    }
    ...
    protected IRubyObject cacheAndCall(IRubyObject caller, RubyClass selfType, IRubyObject[] args, ThreadContext context, IRubyObject self) {
        ...
        rtn = method.call(context, self, selfType, methodName, args);
        if(!RaceDetector.turnOff && !method.shouldInstrument()){
    		RaceDetector.setInstrumentOn();
    	} else if (!RaceDetector.turnOff && method.isCommutativeAnnotated()) {
            RaceDetector.setCommutativeOut();
        ...
        //for commutative check
        instrumentCall(self, method, (IRubyObject)null, rtn);
        return rtn;
    }
    ...
    protected IRubyObject cacheAndCall(IRubyObject caller, RubyClass selfType, ThreadContext context, IRubyObject self) {
        ...
        rtn = method.call(context, self, selfType, methodName);
        if(!RaceDetector.turnOff && !method.shouldInstrument()){
    		RaceDetector.setInstrumentOn();
    	} else if (!RaceDetector.turnOff && method.isCommutativeAnnotated()) {
            RaceDetector.setCommutativeOut();
    ...
    protected IRubyObject cacheAndCall(IRubyObject caller, RubyClass selfType, Block block, ThreadContext context, IRubyObject self) {
        ...
        rtn = method.call(context, self, selfType, methodName, block);
        if(!RaceDetector.turnOff && !method.shouldInstrument()){
    		RaceDetector.setInstrumentOn();
    	} else if (!RaceDetector.turnOff && method.isCommutativeAnnotated()) {
            RaceDetector.setCommutativeOut();
    ...
    protected IRubyObject cacheAndCall(IRubyObject caller, RubyClass selfType, ThreadContext context, IRubyObject self, IRubyObject arg) {
        ...
        rtn = method.call(context, self, selfType, methodName, arg);
        if(!RaceDetector.turnOff && !method.shouldInstrument()){
    		RaceDetector.setInstrumentOn();
    	} else if (!RaceDetector.turnOff && method.isCommutativeAnnotated()) {
            RaceDetector.setCommutativeOut();
        }
        instrumentCall(self, method, arg, rtn);
        return rtn;
    }
    ...
    protected IRubyObject cacheAndCall(IRubyObject caller, RubyClass selfType, Block block, ThreadContext context, IRubyObject self, IRubyObject arg) {
        ...
        rtn = method.call(context, self, selfType, methodName, arg, block);
        if(!RaceDetector.turnOff && !method.shouldInstrument()){
    		RaceDetector.setInstrumentOn();
    	} else if (!RaceDetector.turnOff && method.isCommutativeAnnotated()) {
    	    RaceDetector.setCommutativeOut();
    ...
    protected IRubyObject cacheAndCall(IRubyObject caller, RubyClass selfType, ThreadContext context, IRubyObject self, IRubyObject arg1, IRubyObject arg2) {
        ...
        rtn = method.call(context, self, selfType, methodName, arg1, arg2);
        if(!RaceDetector.turnOff && !method.shouldInstrument()){
    		RaceDetector.setInstrumentOn();
    	} else if (!RaceDetector.turnOff && method.isCommutativeAnnotated()) {
            RaceDetector.setCommutativeOut();
    ...
    protected IRubyObject cacheAndCall(IRubyObject caller, RubyClass selfType, Block block, ThreadContext context, IRubyObject self, IRubyObject arg1, IRubyObject arg2) {
        ...
        rtn = method.call(context, self, selfType, methodName, arg1, arg2, block);
        if(!RaceDetector.turnOff && !method.shouldInstrument()){
    		RaceDetector.setInstrumentOn();
    	} else if (!RaceDetector.turnOff && method.isCommutativeAnnotated()) {
            RaceDetector.setCommutativeOut();
    ...
    protected IRubyObject cacheAndCall(IRubyObject caller, RubyClass selfType, ThreadContext context, IRubyObject self, IRubyObject arg1, IRubyObject arg2, IRubyObject arg3) {
        ...
        rtn = method.call(context, self, selfType, methodName, arg1, arg2, arg3);
        if(!RaceDetector.turnOff && !method.shouldInstrument()){
    		RaceDetector.setInstrumentOn();
    	} else if (!RaceDetector.turnOff && method.isCommutativeAnnotated()) {
            RaceDetector.setCommutativeOut();
    ...
    protected IRubyObject cacheAndCall(IRubyObject caller, RubyClass selfType, Block block, ThreadContext context, IRubyObject self, IRubyObject arg1, IRubyObject arg2, IRubyObject arg3) {
        ...
        rtn = method.call(context, self, selfType, methodName, arg1, arg2, arg3, block);
        if(!RaceDetector.turnOff && !method.shouldInstrument()){
    		RaceDetector.setInstrumentOn();
    	} else if (!RaceDetector.turnOff && method.isCommutativeAnnotated()) {
            RaceDetector.setCommutativeOut();
...
//        }
    ...
    /**
     * This method is added by Weixing Ji for commucativity check
     * */
    protected void instrumentCall(IRubyObject obj, DynamicMethod method, IRubyObject arg, IRubyObject rtn){
    	if(!RaceDetector.turnOff && method.isCommutativeAnnotated()){
    		//System.out.println("InstrumentCall: method " + methodName + " is called");
    		RaceDetector.notifyCall(obj,  method, arg, rtn);
    	}
    }
    protected void instrumentCall(IRubyObject obj, DynamicMethod method, IRubyObject[] args, IRubyObject rtn){
    	if(!RaceDetector.turnOff && method.isCommutativeAnnotated()){
    		//System.out.println("InstrumentCall: method " + methodName + " is called");
    		RaceDetector.notifyCall(obj,  method, args, rtn);
    	}
    }
