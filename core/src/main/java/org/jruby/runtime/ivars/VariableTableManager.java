...
public class VariableTableManager {
    ...
    public long getObjectId(RubyBasicObject self) {
        ...
        /**
         * DPR
         * objectIdAccessor.get is changed to objectIdAccessor.getNoInst for skipping
         * instrumentation
         */
        Long id = (Long)objectIdAccessor.getNoInst(self);
        if (id != null) return id;
        
        synchronized (self) {
            objectIdAccessor = getObjectIdAccessorField().getVariableAccessorForRead();
            id = (Long)objectIdAccessor.getNoInst(self);
            if (id != null) return id;
     ...
     * @param value the value of the variable
     */
    public static void setVariableInternalNoInst(RubyClass realClass, RubyBasicObject self, int index, Object value) {
        if(UnsafeHolder.U == null) {
            SynchronizedVariableAccessor.setVariableNoInst(self,realClass,index,value);
        } else {
            StampedVariableAccessor.setVariableNoInst(self,realClass,index,value);
        }
    }
    ...
    private static void setObjectId(RubyClass realClass, RubyBasicObject self, int index, long value) {
        if (index < 0) return;
        setVariableInternalNoInst(realClass, self, index, value);
    }
